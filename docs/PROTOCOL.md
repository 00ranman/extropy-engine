...
**Calibration replacement is a governed lifecycle.** The calibration behind a domain's measurement operator M_d (its reference sets, fitted parameters, and mapping to bₑ) is not a fixed constant. It is monitored for drift against evidence independent of the calibration's own accepted outputs, and replaced only through a bounded, provenance-carrying, reversible lifecycle. The mint reads M_d only through a CalibrationRecord whose status is active and whose provenance is complete; a missing or incomplete CalibrationRecord fails closed. A proposed replacement runs in shadow mode beside the active calibration and cannot affect mint until independent corroboration is met. Replacement is automatic only within a governed discontinuity bound; a large discontinuity requires wider review rather than silent auto-replacement. If drift is detected without adequate replication, or if independent evidence density falls below the declared minimum, minting through the affected calibration MUST freeze and MUST revert to the last independently corroborated calibration. Outputs accepted by the active calibration cannot be the sole evidence used to validate its successor. The drift thresholds, replacement bounds, epoch length, sample-size minimums, evidence-density minimums, and replication counts are governance-tunable and are specified in [CALIBRATION_LIFECYCLE.md](./CALIBRATION_LIFECYCLE.md). No numeric defaults are set for them in this release.
