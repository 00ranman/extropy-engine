...
Two domain vocabularies exist in the repository's history, and this Codex reconciles them explicitly because the collision has caused real confusion.

`docs/SPEC_v3.1.md` §5 and the underlying contribution-graph literature use an eight-domain set organized around contribution style: cognitive, code, social, economic, thermodynamic, informational, governance, and temporal. This set is useful for categorizing what kind of work a contribution represents, and it remains in active use in the reference implementation's rarity tables and contribution-graph routing.

`docs/NORMALIZATION.md`, which governs the mint-side cross-domain arithmetic, uses a different eight-domain set: thermodynamic, informational, social, economic, ecological, governance, cognitive, and spiritual (reserved, non-minting). This is the set that the bₑ normalization pipeline actually normalizes over, and it is the set this Codex treats as canonical for §5 through §8.

The two sets overlap substantially (thermodynamic, informational, social, economic, governance, cognitive appear in both) and differ in two places: the contribution-style set includes code and temporal, which are treated in the normalization set as sub-instances of informational and governance-adjacent measurement respectively, while the normalization set includes ecological and the reserved spiritual slot, which the contribution-style set omits. Where this Codex needs to refer to "the eight domains" without qualification, it means the `NORMALIZATION.md` set, because that is the set the mint layer actually normalizes over. Where a specific contribution-graph routing rule references code or temporal explicitly, this Codex says so and treats those as domain-adjacent categorizations rather than separate mint-side domains. This reconciliation is itself part of the naming-hygiene program described in §4.7.

### 4.5 Why Ecological and the Reserved Spiritual Slot Are Handled Differently

Ecological entropy has a defensible measurement operator (Shannon entropy of the ecosystem-state distribution over species, stocks, or flows in a bounded region; see `docs/NORMALIZATION.md` §3.5) and is adopted as a full mint-side domain. The reserved spiritual slot has no accepted measurement operator. It remains in the `EntropyDomain` enum for schema stability, meaning code that switches over domain values does not need to handle an undefined case if the slot is ever populated, but it MUST NOT mint XP under the canonical formula until a domain-appropriate M_d is defined, tested, and adopted through governance (`docs/NORMALIZATION.md` §3.8). This is a deliberate act of intellectual honesty: rather than force a spiritual-domain instrument into existence to complete a tidy list, the specification leaves the slot open and inert.
...
### 8.1 The Canonical Eight, Reconciled

As established in §4.4, this Codex treats the `docs/NORMALIZATION.md` domain set as canonical for mint-side purposes: thermodynamic, informational, social, economic, ecological, governance, cognitive, and a reserved spiritual slot. Where the contribution-graph literature's alternate set (which includes code and temporal as separate categories) is operationally relevant, this section notes the mapping explicitly.

### 8.2 Thermodynamic

Physical disorder: waste heat, physical inefficiency, environmental degradation, energy loss. Typical contributions include improving insulation, recycling systems, waste reduction in physical production, and environmental restoration. The measurement operator computes the Shannon entropy of the phase-space distribution before minus after, expressed in bits, anchored to Landauer's principle: 1 bₑ of thermodynamic entropy erased corresponds to k_B T ln 2 joules of minimum dissipation avoided (`docs/NORMALIZATION.md` §3.1). Typical instruments include energy-use deltas, heat-loss reduction measurements, material recovery rates, and emissions changes.

### 8.3 Informational

Disorder in records, data quality, accessibility, signal-to-noise ratio, and archival coherence. Typical contributions include cleaning a dataset, organizing records, fact-checking and source reconciliation, and improving discoverability. The measurement operator computes the Shannon entropy of the document, channel, or dataset representation, before minus after, anchored to raw Shannon bits with H₀ equal to 1. For prediction-loop domains, where the mint is contingent on a specific claim being verified, such as a proof or a merged patch, the operator instead uses the log-likelihood-ratio of the claim under the pre-verification distribution (`docs/NORMALIZATION.md` §3.2). This is the domain the `github-parasite` package operates in; see §15.

### 8.4 Social

Disorder in trust networks, cooperation, conflict dynamics, and community coherence. Typical contributions include mediation, trust restoration, organizing a fractured group, and reducing communication breakdown. The measurement operator computes the Shannon entropy of the assignment distribution over N agents, before minus after, anchored to the idea that one bₑ of social entropy reduction corresponds to removing one round of who-does-what ambiguity in a coordination game (`docs/NORMALIZATION.md` §3.3).

### 8.5 Economic

Disorder in allocation, throughput, matching, waste, bottlenecks, and coordination of scarce resources. Typical contributions include better matching supply to need, removing a useless intermediate step, improving workflow efficiency, and reducing idle capacity. The measurement operator computes the Shannon entropy of the allocation or price-discovery distribution, before minus after, anchored to one bₑ of economic entropy reduction meaning resolving one bit of allocation-outcome uncertainty in a market or matching. This is never denominated in fiat; see the Non-Extraction invariant in §13 (`docs/NORMALIZATION.md` §3.4).

### 8.6 Ecological

Disorder in ecosystem state: species presence, stocks, and flows in a bounded region. The measurement operator computes the Shannon entropy of the ecosystem-state distribution before minus after, anchored to one bₑ of ecological entropy reduction meaning resolving one bit of ecosystem-state uncertainty by measurement or restoration (`docs/NORMALIZATION.md` §3.5). This domain is adopted in full, unlike the reserved spiritual slot, because a defensible M_d exists.

### 8.7 Governance

Disorder in decision systems, accountability structures, legitimacy, responsiveness, and rule coherence. Typical contributions include fixing a broken decision process, making accountability enforceable, reducing policy contradiction, and improving transparency. The measurement operator computes the Shannon entropy of the decision distribution under a codified rule set, before minus after, anchored to one bₑ of governance entropy reduction meaning removing one bit of decision uncertainty under that rule set (`docs/NORMALIZATION.md` §3.6). Typical instruments include decision latency, reversal frequency, participation quality, and auditability metrics.

### 8.8 Cognitive

Disorder in belief states, knowledge, understanding, mental models, skill formation, and conceptual coherence. Typical contributions include teaching a concept clearly, correcting a misconception, building a curriculum, and producing documentation that improves comprehension. The measurement operator computes the Shannon entropy of the agent's belief state, before minus after, anchored to one bₑ of cognitive entropy reduction meaning removing one bit of belief-state uncertainty in a verifiable epistemic agent, such as a test scored, a model's calibration improved, or a misconception corrected in a way validators can inspect (`docs/NORMALIZATION.md` §3.7).

### 8.9 The Reserved Spiritual Slot

Not adopted. Retained in the `EntropyDomain` enum purely for schema stability, so that code switching over domain values does not need special-case handling if the slot is ever populated. There is no accepted M_spiritual. This domain MUST NOT mint XP under the canonical formula until such an operator is defined, tested, and adopted through governance (`docs/NORMALIZATION.md` §3.8). See §4.5 for the reasoning behind treating this slot differently from the ecological domain.

### 8.10 Code and Temporal as Cross-Cutting Categories

The contribution-graph literature's alternate domain vocabulary includes code and temporal as distinct categories. Under the reconciliation in §4.4, code-related contribution is treated as a specialized instance of informational entropy reduction (fixing a bug, refactoring a brittle module, increasing test coverage, reducing complexity, measured via cyclomatic complexity, test pass rates, error frequency, and static analysis scores), and temporal contribution (better scheduling, queue reduction, workflow synchronization, eliminating dead time, measured via cycle time, wait time, and scheduling-conflict reduction) is treated as a specialized instance of governance and social entropy reduction depending on context. Both remain useful operational categorizations inside the contribution graph's rarity tables even though they are not independent mint-side domains under the current normalization architecture.

### 8.11 Intersectionality and Domain Vectors

Real contributions land across multiple domains at once. A teacher may reduce cognitive, social, and governance-adjacent disorder in a single act. A clean software deployment may reduce informational, economic, and ecological disorder simultaneously. The engine does not force a claim into one exclusive box; it measures a domain vector E across all eight dimensions and weights it contextually through the governance-set vector w, as specified in §5.5.

### 8.12 Why Not More Domains

Adding a ninth domain requires demonstrating three things: that the proposed domain admits a measurement instrument grounded in either the Landauer correspondence or a defensible Shannon-entropy analogue; that the proposed domain's measurement cannot already be expressed under one of the existing eight without loss; and that the proposed domain has a falsifiability condition definable in advance, per `docs/NORMALIZATION.md` §4's invariants. No domain has cleared all three bars beyond the current eight, and the reserved spiritual slot exists precisely because it has not cleared the first bar.

### 8.13 Operationalization Status

As of this writing, no domain has a fully mature, governance-adopted calibration table with production-grade validator-neighborhood recipes. The near-term roadmap (see §19) prioritizes landing at least three domain operators with calibration tables and validator-neighborhood recipes, with the informational domain first because it maps most directly onto the reference implementation's current evidence surface through `packages/github-parasite`. This is stated plainly rather than implied, because the gap between "a domain is named in this Codex" and "a domain has a working, adversarially tested M_d" is exactly the gap the falsification conditions in §7 (Concise edition, restated as §6.2 of this Codex below) are designed to surface honestly.
...
Thermodynamic, informational, social, economic, ecological, governance, cognitive: seven adopted mint-side domains, each with a stated measurement operator M_d. See §8.2 through §8.8. Spiritual: a reserved, non-minting slot retained for schema stability. See §8.9. Code and temporal: cross-cutting contribution-graph categorizations, not independent mint-side domains under the current normalization architecture. See §8.10.
...
### T.7 Ecological Entropy: Deep Dive

Ecological entropy is disorder in ecosystem state: species presence, stocks, and flows within a bounded region, per `docs/NORMALIZATION.md` §3.5. Operationalization paths include biodiversity-index tracking, measuring a documented change in species richness or evenness within a defined area; resource-stock monitoring, tracking a measured recovery in a specific resource stock, such as a fishery or an aquifer, against a documented depletion baseline; and restoration-outcome verification, confirming that a specific ecological intervention, a wetland restoration or a reforestation effort, produced a measurable, sustained change in the target ecosystem's state. This domain's known challenge is measurement cost and latency: ecological state changes often unfold over years, which sits uneasily with the protocol's default two-to-five-minute task grain (§9.4), and ecological-domain claims are accordingly expected to route through the speculative and exploratory stakes class described in §9.2 more often than through the everyday and habitual class.

### T.8 Temporal Contribution as a Cross-Cutting Category

As established in §8.10, temporal contribution, better scheduling, queue reduction, workflow synchronization, is treated as a cross-cutting categorization within the contribution graph's rarity tables rather than as an independent mint-side domain under the current normalization architecture. Operationalization paths, where temporal contribution is scored, include cycle-time reduction, measured as a documented decrease in the time from task initiation to completion for a repeated process, and wait-time reduction, measured as a documented decrease in idle time within a scheduled system. These measurements typically feed into the governance or social domain vectors depending on context, per the intersectionality treatment in §8.11, rather than accumulating a separate temporal ΔS_bₑ term.
