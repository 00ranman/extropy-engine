...
log "Writing nginx site config at $NGINX_SITE"
cat > "$NGINX_SITE" <<EOF
# Generated by deploy-homeflow.sh, do not edit by hand
# Reverse proxy to HomeFlow on localhost:$APP_PORT
server {
    listen 80;
    listen [::]:80;
    server_name $DOMAIN;
...
server {
    ...
    # Certs come from certbot, see post-deploy notes
...
1) DNS:
   ...
   Wait until it resolves. Test with:
     dig +short homeflow.extropyengine.com
   Should print 187.124.95.129.

2) SSL cert (run after DNS resolves):
